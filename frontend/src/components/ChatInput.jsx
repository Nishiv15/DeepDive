...
export function ChatInput({ onSend, disabled, documentId }) {
  ...
  return (
    <div className="border-t border-[var(--color-border)] bg-[var(--color-surface)] p-4 space-y-3">
      {/* Options panel */}
      {showOptions && (
        <div className="glass rounded-lg p-3 space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-200">
          <div className="flex items-center justify-between">
            <label className="text-xs font-medium text-[var(--color-text)]">Chunks to retrieve (top-K)</label>
      ...
      <div className={cn(
        ...
        <textarea
          ...
          placeholder={documentId ? 'Ask anything about the document…' : 'Upload a document first'}
          className="flex-1 resize-none bg-transparent text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] focus:outline-none leading-relaxed py-1 px-2 max-h-40 overflow-y-auto"
        />
